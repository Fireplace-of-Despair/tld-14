...
namespace TLD14.Pages.Components.IndexProjects;

public sealed class IndexProjects(IStorage storage) : ViewComponent
{
    #region DataModel
    ...
    public IndexProjectsModel? Model { get; set; }

    public async Task<IViewComponentResult> InvokeAsync()
    {
        Model = await storage.LoadAsyncEx<IndexProjectsModel, IndexProjectsModel.Translation>
        (
            HttpContext.GetRequestLanguage()
        );
        Model.Properties!.Projects =
            Model.Properties.Projects.Where(x => x.DivisionKey != Constants.Divisions.AshenChronicles)
            .ToList();
